...
public class ThemeController {
    ...
    public Response<Theme> updateUsers(@PathVariable("id") Long id, @RequestBody Theme c) {
        ...
        @PutMapping("/updateThemeProgress/{id}")
    public Response<Theme> updateProgress(@PathVariable("id") Long id, Theme c) {
        try {
            ...
            c.setFormation(c.getFormation() == null ? oldUser.getFormation() : c.getFormation());
            c.setId(id);
            c.setProgressByTheme(true);


            return new Response<Theme>("200","Video updated", iTheme.save(c));
        }catch (Exception e){
            return new Response<Theme>("406", e.getMessage(), null);
    ...
    @GetMapping("/getthemeActive/{id}/{progressByEtape}")
    public Response<List<Theme>> progessTheme( @PathVariable Long id,@PathVariable Boolean progressByEtape) {


        try {
            return new Response<List<Theme>>("200", "progress", iTheme.getThemeeActive(id,progressByEtape));

        } catch (Exception e) {
            return new Response<List<Theme>>("406", e.getMessage(), null);
        }
