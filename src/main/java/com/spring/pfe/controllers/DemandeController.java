...
public class DemandeController {
  ...
  //  t.setFormationn(f);
    ...
    @GetMapping("/Nottactive/{active}/{vu}")
    public Response<List<Demande>> GetAllDemandeNotActiveWITHVuFalse( @PathVariable("active") Boolean active,@PathVariable("vu") Boolean vu) {
        try {



            return new Response<List<Demande>>("200", "Get all User by formaion", idemande.getAllNotActiveVuFalse(active,vu));
    ...
    @GetMapping("/demande/{id}/{active}")
    public Response<List<Demande>> GetAllFormationNotActiveByUser(@PathVariable Long id,@PathVariable Boolean active){

        try {

            return new Response<List<Demande>>("200", "Get all formation",  idemande.getAllFormationActiveByUser(id,active));
...
//Demande d = idemande.getAllsss(users_id);
    ...
    @PutMapping("/VueTrue/{id}")
    public Response<Demande> updateVu(@PathVariable("id") Long id,@RequestBody Demande d) {
        try {
            Demande oldDemande = idemande.findById(id).orElse(null);
            d.setUsers(d.getUsers() == null ? oldDemande.getUsers() : d.getUsers());
            d.setFormationn(d.getFormationn() == null ? oldDemande.getFormationn() : d.getFormationn());
            ...
            return new Response<Demande>("200","demande updated", idemande.save(d));
        }catch (Exception e){
            return new Response<Demande>("406", e.getMessage(), null);
